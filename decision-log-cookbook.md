...
## Operations / alerting runbook

Every operational signal fivedrisk emits, where it lands, and a recommended alert.
"Event stream" = the NDJSON channel (`configure(event_path=...)`); "audit log" =
the SQLite `decisions` table.

| Signal | Where it lands | Recommended alert |
|---|---|---|
| **RED decision (BLOCK)** | audit log row `band='RED'`; hook returns `decision='block'` | Alert on any `RED` in a window; page on a burst. |
| **ORANGE decision (approval required)** | audit log row `band='ORANGE'` | Track ORANGE rate; alert if the HITL queue is not draining. |
| **Egress block** (leakage / injection-echo / semantic-review on tool output) | audit log row `source='post-tool-egress'`, `band='RED'` (via `log_egress_block`) | Alert on any egress block — it means data almost left. |
| **Budget cap exceeded** | event stream `event_type='budget_intervention'`, `reason_code=BUDGET_CAP_EXCEEDED`; `@gate` raises `BudgetExceededError` | Alert per session hitting the cap; dashboard cap-hit rate. |
| **Reservation blocked** | event stream `reason_code=BUDGET_RESERVATION_BLOCKED` | Same channel as cap-exceeded; usually a noisy-agent signal. |
| **Identity required, not supplied** | event stream `event_type='identity_required_denial'`, `reason_code=IDENTITY_REQUIRED_NOT_SUPPLIED` | Alert — a caller is not passing identity where policy requires it. |
| **SafetyDrift escalation** | audit log rationale contains `[SafetyDrift: markov_absorption: ...]` | Alert on drift bumps; they mark a session trending dangerous. |
| **Fail-closed / observer or log unavailable** | `RuntimeWarning` on stderr (e.g. DecisionLog fell back to a temp path) | Alert on the warning — the audit trail is degraded. |
| **Injection detected (input)** | hook returns `decision='block'`, reason `5D injection detected...` | Alert; correlate with the session/trace id. |
| **Unparsed / error rate** | gateway emits `{"error", "error_type"}` lines; CLI exits non-zero | Alert if the error rate rises — indicates a malformed integration. |

Wire the event stream into your SIEM by tailing the NDJSON file; each event carries
`trace_id` / `session_id` for correlation with your agent runs.

---

*Aligned with the bundled `DecisionLog` schema. Queries tested against the shipped `DecisionLog`. Taxonomy: `docs/spec/taxonomy.md`. Deployer setup: `docs/spec/deployment.md`.*
