...
DETECTOR_CORPUS_VERSION = "2026-09-10.1"

# Qualifier run shared by the `ignore` and `disregard` patterns.
#
# 🔴 It exists because the two patterns drifted apart and BOTH were wrong (2026-09-10):
#   * `ignore` accepted exactly ONE qualifier, so `ignore previous instructions` matched
#     while `ignore all previous instructions`, the single most quoted phrasing of the
#     attack, did not.
#   * `disregard` wrote its alternation as `(?:previous|prior\s+)?`, attaching the
#     trailing space to `prior` and not to `previous`. So `disregard prior instructions`
#     matched and `disregard previous instructions` did not. Two words, one alternation,
#     different behaviour.
#
# Both were found by pointing the shipped corpus at adversarial strings from a live
# deployment rather than from the test fixtures. One fragment, one place to fix.
_QUALIFIER = r"(?:the|all|any|your|my|these|those|previous|prior|earlier|preceding|above|foregoing)\s+"

# 🔴 THE NOUN IS A SLOT TOO, AND SO IS THE QUANTIFIER. Sharing _QUALIFIER stopped the two verbs
# drifting on the qualifier and left them drifting on everything else: `ignore` took (?:Q)+ and
# `disregard` took (?:Q)*, so "ignore instructions" MISSED while "disregard instructions" HIT.
# Measured 2026-09-11. The same defect one level up, in the fix for it.
# Composition, not enumeration: VERB x QUALIFIER-RUN x NOUN, each a set, the run unbounded and
# OPTIONAL for every verb. Enumerating phrasings is how "ignore all previous instructions", the
# most quoted form of the attack, shipped unmatched.
_NOUN = (r"(?:instructions?|prompts?|rules?|rulesets?|context|system|guidance|guidelines?"
         r"|directions?|directives?|orders?|commands?|constraints?|policy|policies)")
_IGNORE_VERB = r"(?:ignore|disregard|overlook|bypass|skip)"

# Input-side prompt-injection / hostile instruction corpus.
INJECTION_PATTERNS: list[tuple[str, str]] = [
    (r"(?i)\b" + _IGNORE_VERB + r"\s+(?:" + _QUALIFIER + r")*" + _NOUN + r"\b", "override"),
