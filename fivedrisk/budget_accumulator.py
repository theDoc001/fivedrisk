...
class BudgetAccumulator:
    ...
    # Serializes the check-then-act in reserve/commit/cancel so concurrent
    # callers (e.g. parallel @gate interceptors / MCP calls) cannot race past
    # the cap via a stale projected_total read. Not part of equality/repr.
    ...
    def reserve_for_tool_call(
        ...
        Thread-safe: the read-project-decide-mutate sequence runs under a lock
        so concurrent reservations cannot both pass a stale cap check (S3).
        """
        with self._lock:
