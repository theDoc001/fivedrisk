...
class TestBudgetEnforcementAtGate:
    ...
    def test_destination_block_cancels_sync_budget_reservation(self, tmp_event_path) -> None:
        configure(
            event_path=str(tmp_event_path),
            destination_allowlist=["allowed.example"],
        )
        from fivedrisk import hooks as h
        h._policy = Policy(
            max_session_budget_tokens=10_000,
            max_tool_call_budget_tokens=100,
        )

        @gate(tool_name="Bash", estimated_input_tokens=1000)
        def run_command(command: str, session_id: str) -> str:
            return "should never run"

        # Sibling fail-open fix: a destination block is a FivedriskDenial
        # (DestinationBlockError), NOT a swallowable ValueError.
        with pytest.raises(DestinationBlockError):
            run_command("curl https://blocked.example", session_id="s1")

        snapshot = h._budget_accumulators["s1"].snapshot()
        assert snapshot["pending_reservations"] == 0
        assert snapshot["pending_tokens"] == 0


@pytest.mark.asi03_identity_abuse
@pytest.mark.asi07_inter_agent_comm
