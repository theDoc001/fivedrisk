...
pytestmark = [
    pytest.mark.safety_drift,
    pytest.mark.asi08_cascading,
    pytest.mark.asi10_rogue_agent,
]
...
class TestDriftCapability:
    ...
    def test_unobserved_absorbing_state_gets_identity_row(self):
        # M16: with only a transient-state observation, unobserved ABSORBING
        # states must be identity rows (self-loop), not uniform. A uniform row
        # would make is_absorbing() disagree with the matrix.
        matrix = build_transition_matrix([[(0, 0), (0, 1)]])
        for idx in range(len(matrix)):
            data_tier, activity_tier = index_to_state(idx)
            if is_absorbing(data_tier, activity_tier):
                # unobserved absorbing state -> pure self-loop
                assert matrix[idx][idx] == 1.0
                assert abs(sum(matrix[idx]) - 1.0) < 1e-9
                assert sum(1 for v in matrix[idx] if v != 0.0) == 1

    def test_input_validation_survives_python_O(self):
        # M17: range validation must raise even under `python -O` (asserts are
        # stripped there). Run a child interpreter with -O and confirm the
        # ValueError still fires instead of silently producing garbage.
        import subprocess
        import sys

        code = (
            "from fivedrisk.markov import state_to_index\n"
            "try:\n"
            "    state_to_index(9, 0)\n"
            "    print('NO_RAISE')\n"
            "except ValueError:\n"
