...
class TestClassifierCapability:
    ...
    def test_docker_wrapped_rm_rf_does_not_downgrade_risk(self):
        # H1 regression: a destructive command must NOT score lower just because
        # it also matches a second, weaker pattern. Default overrides iterate
        # "rm -rf" {TP:4,R:4} before "docker" {TP:3,R:2}; last-write-wins would
        # downgrade the wrapped command to TP:3,R:2 (RED -> ORANGE). Per-dimension
        # max must keep the spike.
        policy = Policy()
        plain = classify_tool_call("Bash", {"command": "rm -rf /data"}, policy=policy)
        wrapped = classify_tool_call(
            "Bash", {"command": "docker exec app rm -rf /data"}, policy=policy
        )
        # Wrapped must score >= unwrapped on every dimension of the spike.
        assert wrapped.tool_privilege >= plain.tool_privilege
        assert wrapped.reversibility >= plain.reversibility
        assert wrapped.tool_privilege == 4
        assert wrapped.reversibility == 4
        # And the band must not be downgraded by the wrapping.
        assert score(wrapped, policy).band == score(plain, policy).band

    def test_bash_override_merge_takes_per_dimension_max(self):
        # Two overlapping patterns on the same dimension: the higher value wins
        # regardless of insertion order.
        policy = Policy(
            bash_overrides={
                "alpha": {"tool_privilege": 1},
                "beta": {"tool_privilege": 4},
            }
        )
        low_first = classify_tool_call(
            "Bash", {"command": "alpha then beta"}, policy=policy
        )
        assert low_first.tool_privilege == 4
