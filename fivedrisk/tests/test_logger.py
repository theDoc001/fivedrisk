...
def test_decisionlog_recovers_from_corrupted_db(tmp_path, monkeypatch):
    """A corrupted / non-SQLite file at the log path must not crash DecisionLog.

    Regression: ``sqlite3`` raises a bare ``DatabaseError`` ("file is not a
    database") on a corrupted file, which the constructor previously did not
    catch (it caught only ``OperationalError``). It must fall back to a writable
    temp DB and remain usable.
    """
    import tempfile

    # Isolate the fallback temp dir so a stray temp DB cannot interfere.
    monkeypatch.setattr(tempfile, "gettempdir", lambda: str(tmp_path))
    corrupt = tmp_path / "corrupt.db"
    corrupt.write_bytes(b"this is not a sqlite database, just garbage bytes")

    log = DecisionLog(str(corrupt))  # must not raise

    assert log.fallback_active is True
    # The log remains usable after recovery.
    entry_id = log.log(_make_scored())
