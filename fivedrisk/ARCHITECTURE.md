...
**Maps**: the 5D governance model → `fivedrisk/` code → backlog
**Updated**: 2026-04-13
**Version**: 0.3.0
...
| §15.7 User Levers | Prompt / Risk | ⚠️ PARTIAL | `router.py` | — |
| Risk-based model routing | Higher risk → higher model floor | ✅ DONE | `scorer.py`, `router.py` | Enforced in routing |
...
│ • Never used for routine work (reserved for high-risk actions)│
└─────────────────────────────────────────────────────────────┘
```
