...
def state_to_index(data_tier: int, activity_tier: int) -> int:
    ...
    # M17: input validation raises (survives `python -O`), not asserts.
    if not 0 <= data_tier < TIER_COUNT:
        raise ValueError(f"data_tier out of range: {data_tier}")
    if not 0 <= activity_tier < TIER_COUNT:
        raise ValueError(f"activity_tier out of range: {activity_tier}")
...
def index_to_state(idx: int) -> tuple[int, int]:
    ...
    if not 0 <= idx < STATE_COUNT:
        raise ValueError(f"state index out of range: {idx}")
...
def build_transition_matrix(
    ...
    for from_idx, row in enumerate(counts):
        total = sum(row)
        if total == 0.0:
            # M16: an unobserved ABSORBING state must stay absorbing (identity /
            # self-loop row). A uniform row would let is_absorbing() disagree with
            # the actual matrix, so the Q/R decomposition in
            # compute_absorption_probabilities models a chain that contradicts it.
            # Unobserved transient states fall back to uniform.
            if is_absorbing(*index_to_state(from_idx)):
                identity_row = [0.0] * STATE_COUNT
                identity_row[from_idx] = 1.0
                matrix.append(identity_row)
            else:
                matrix.append(uniform_row.copy())
        else:
            matrix.append([value / total for value in row])

    if not all(abs(sum(row) - 1.0) < ROW_SUM_TOLERANCE for row in matrix):
        raise ValueError("constructed transition matrix has a non-stochastic row")
    return matrix


def matrix_distance(m1: list[list[float]], m2: list[list[float]]) -> float:
    """Distance between two row-stochastic transition matrices (fitted vs baseline).

    Row-wise L1 averaged over the rows: ``(1/N) * sum_ij |m1[i][j] - m2[i][j]|``.
    A plain, deterministic distance used only as an offline DRIFT signal — how far a
    recently-fitted chain has moved from the calibrated-baseline chain. Properties
    (asserted by the M3 golden):
      * ``0.0`` for identical matrices,
      * ``>= 0.0`` and symmetric,
      * monotonic: more probability mass moving toward high/absorbing tiers => a
        strictly larger distance from the calm baseline.
    """
    if not m1 or not m2:
        raise ValueError("matrices must be non-empty")
    n = len(m1)
    ...
    if any(len(r) != n for r in m1) or any(len(r) != n for r in m2):
        raise ValueError("matrices must be square and the same shape")
    total = 0.0
    for row1, row2 in zip(m1, m2):
        total += sum(abs(a - b) for a, b in zip(row1, row2))
    return total / n
...
def make_default_transition_matrix() -> list[list[float]]:
    ...
    if not all(abs(sum(row) - 1.0) < ROW_SUM_TOLERANCE for row in matrix):
        raise ValueError("default transition matrix has a non-stochastic row")
    return matrix
...
def compute_absorption_probabilities(
    ...
    # M17: user-supplied matrix — validate stochastic rows with a raise, not an
    # assert, so a malformed matrix cannot silently produce garbage under -O.
    if not all(abs(sum(row) - 1.0) < ROW_SUM_TOLERANCE for row in transition_matrix):
        raise ValueError("every transition matrix row must sum to 1.0")
    ...
    if len(transient_states) != 10:
        raise ValueError(f"expected 10 transient states, got {len(transient_states)}")
    if len(absorbing_states) != 6:
        raise ValueError(f"expected 6 absorbing states, got {len(absorbing_states)}")
    ...
    for row_idx, state_idx in enumerate(transient_states):
        ...
        if expected_steps <= 0.0:
            raise ValueError("expected absorption steps must be positive")
...
class MarkovDriftTracker:
    ...
    def record(self, scored_action: Any) -> Optional[DriftBump]:
        ...
        # Low-11: always feed the counter-based fallback so its state keeps
        # advancing even in the high-risk regime. Previously, when the Markov
        # threshold fired but the action was already at/above the target band,
        # record() returned None WITHOUT calling the fallback, silently freezing
        # counter state exactly when drift matters most.
        fallback_bump = self._fallback.record(scored_action)
        ...
        if target_band is not None:
            # Markov fired but the action already meets the target band: no extra
            # escalation. The fallback's state was still advanced above (Low-11),
            # preserving the original return contract for this branch.
            return None

        # Markov did not fire; surface any counter-based bump from the fallback.
        return fallback_bump
