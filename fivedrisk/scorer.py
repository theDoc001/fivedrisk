...
def score(action: Action, policy: Policy | None = None) -> ScoredAction:
    ...
    # is a 3-band experience (GREEN / ORANGE / RED). Deployments that need
    # the moderate-risk tier opt in via policy.enable_yellow_band = True.
    if band == Band.YELLOW and not policy.enable_yellow_band:
        band = Band.GREEN

    # Step 5c: Policy floor. A floored action is at least its floor band
    # regardless of the computed score, and cannot be lowered at runtime (this
    # only ever raises the band). Applied here, before routing, so routing and
    # rationale reflect the enforced band.
    floor_rule = policy.matched_floor(action.tool_name, action.tool_input)
    if floor_rule is not None:
        band = max(band, floor_rule.band, key=lambda b: band_order.index(b))
    ...
    if floor_rule is not None:
        floor_note = floor_rule.reason or f"floor:{floor_rule.band}"
        rationale = f"{rationale} [{floor_note}]"
    ...
    return ScoredAction(
        ...
        routing=routing,
        # Structured floor attribution alongside the prose note above. The prose is
        # kept verbatim (callers format it into STOP/ASK messages — hooks.py:660,791);
        # these carry the same fact in a queryable form. `id` defaults to "" on
        # FloorRule, which normalises to None: absent, not empty.
        fired_rule_id=(floor_rule.id or None) if floor_rule is not None else None,
        floor_band=str(floor_rule.band) if floor_rule is not None else None,
    )
