...
_IRREVERSIBLE_PATTERNS: list[tuple[str, str]] = [
    ...
    # Low-2: dropped the `(?!\s+where)` lookahead — it was defeated by `\w+`
    # backtracking (a WHERE-scoped statement still matched) and redundant with the
    # delete/drop pattern above. Broadening to also flag WHERE-scoped writes is the
    # safe direction for a reversibility signal (a scoped DELETE is still a write).
    (r"(?i)((update|insert|delete)\s+(?:from|into)?\s*\w+)", "reversibility"),
