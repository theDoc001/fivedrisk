...
def measure(fn, n: int = 2000) -> tuple[float, float, float]:
    ...
    # Low-14: sort a copy; nearest-rank index ceil(q*n)-1 clamped (was one high).
    s = sorted(samples)
    m = len(s)

    def idx(q: float) -> int:
        return min(m - 1, max(0, math.ceil(q * m) - 1))

    return (s[idx(0.50)] / 1000, s[idx(0.95)] / 1000, s[idx(0.99)] / 1000)
...
def main() -> None:
    ...
    print()
    print("These are fivedrisk's own numbers on this machine. This script prints no")
    print("timing for any other tool: measure yours on your own traffic.")
